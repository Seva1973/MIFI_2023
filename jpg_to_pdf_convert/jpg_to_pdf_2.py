...
def convert_and_replace(file_path, src_dir, dst_dir):
...
def recursive_convert(dir_path, src_dir, dst_dir):
    for item in os.listdir(dir_path):
        item_path = os.path.join(dir_path, item)
        if os.path.isdir(item_path):
            recursive_convert(item_path, src_dir, dst_dir)
        else:
            convert_and_replace(item_path, src_dir, dst_dir)
